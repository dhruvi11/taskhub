...
import { getFCMToken } from "./firebase.service";
...
export async function registerDeviceToken(accessToken: string) {
  if (Platform.OS === "web") {
    console.log("Device token registration is not available on web");
  ...
  if (!response.ok) {
    ...
    throw new Error(`Device token registration failed: ${error}`);
  }
