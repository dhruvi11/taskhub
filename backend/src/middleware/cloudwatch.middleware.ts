...
export const cloudWatchRequestMiddleware = (
  ...
  res.on("finish", () => {
    ...
    void (async () => {
      try {
        await cloudWatchService.log(level, "API request", {
          method: req.method,
          route,
          statusCode,
        ...
        await cloudWatchService.metric("RequestCount", 1, "Count", {
          Method: req.method,
        });
        ...
        if (statusCode >= 400) {
          await cloudWatchService.metric("ErrorCount", 1, "Count", {
            ErrorType: statusCode >= 500 ? "5xx" : "4xx",
          });
        }
      } catch (error) {
        console.error("CloudWatch request logging failed:", error);
      }
    })();
  });
